...
var utils = require('./utils')
  ...
  , os = require('os')
  , fontRuleParser = require('css-font-parser')
...
function inlineDocument(document, css) {
  ...
  var isSVGTextElement = (function(){
    var SVGTextElements = [ 'altGlyph', 'textPath', 'text', 'tref', 'tspan' ];
    
    return function(el) {
      return SVGTextElements.indexOf(el.tagName.toLowerCase()) != -1;
    };
  }());
  ...
  function handleRule(rule) {
    ...
    function setStyleAttrs(el) {
      var style = '';
      for (var name in el.styleProps) {
        if (/^font(?:\-\w+)*$/.test(name) && isSVGTextElement(el)) {
          var val = el.styleProps[name].value;
          if (name === 'font') {
            var fontProps = fontRuleParser(val);
            for (var prop in fontProps) {
              var val = fontProps[prop];
          ...
          style += el.styleProps[name].toString() + ' ';
        }
      }
